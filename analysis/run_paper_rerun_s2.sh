...
EXPECTED_BRANCH="${EXPECTED_BRANCH:-paper-handoff-airtight-20260825}"
...
MODEL_ASSET_TOOL="$SCRIPT_DIR/prepare_model_assets.py"
...
REFERENCE_DIR="$REPO_DIR/paper_outputs/reference_metrics_20260810"
...
REFERENCE_ENV_FREEZE="$REFERENCE_DIR/pip_freeze.txt"
...
preflight() {
    ...
    require_path "$PY"
    require_path "$MODEL_ASSET_TOOL"
    if [[ ! -s "$VICREG_CKPT/epoch_1000.ckpt" || ! -s "$IJEPA_CKPT/epoch_1000.ckpt" ]]; then
        fail "Published CelebA checkpoints are missing. Run: $PY -m analysis.prepare_model_assets --asset-root $ROOT"
    fi
    ...
    (
        cd "$REPO_DIR"
        "$PY" -m analysis.prepare_model_assets --asset-root "$ROOT" --verify
    )
    ...
    printf 'Preflight passed: branch, commit, inputs, CUDA, disk, and syntax.\n'
}
