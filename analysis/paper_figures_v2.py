...
Main figures
  ...
  7  the capture-form bound in high-capture 3DShapes representations
...
def _target_points(model_dir: Path, shot: int, cache: Path | None) -> list[dict[str, Any]]:
    ...
    if cache is not None:
        with (cache / f"{model_dir.name}_shot{shot}.json").open(
            "w", encoding="utf-8", newline="\n"
        ) as handle:
            handle.write(json.dumps(points))
    return points


# ------------------------------------------------------------------ main 1
def fig1_augmentation(survival_csv: Path, stem: Path) -> list[Path]:
    ...
    Each of four pairing conditions is trained independently at seeds 6, 17,
    and 29 (twelve model runs total). Panel (a) contains twelve condition-task
    trajectories after averaging each one across its three seeds. The three
    demoted condition-task paths collapse to zero; the other nine paths remain
    pinned at one. Panel (b) is the same result as the design matrix, so the
    ablation structure is explicit. In particular, "nine" counts plotted
    condition-task paths, not latent factors or models.
    ...
    for row in rows:
        series[(row["condition"], row["task"], int(row["epoch"]))].append(_f(row["capture_B"]))

    fig, axes = plt.subplots(1, 2, figsize=(10.2, 3.85), gridspec_kw={"width_ratios": [1.32, 1.0]})
    ...
    for condition in CONDITION_ORDER:
        for task in tasks:
            means = [mean_range(series[(condition, task, e)])[0] for e in epochs]
    ...
    for condition, task in varied_by_condition.items():
        stats = [mean_range(series[(condition, task, e)]) for e in epochs]
    ...
    heading(ax, "Four trained pairing conditions selectively discard one factor")
    ygrid(ax)
    ...
    direct_label(
        ...
        final + 3,
        1.0,
        "9 shared\ncondition-task paths",
        SLATE,
        size=8.5,
        bold=True,
    )
    for task, y in zip(tasks, (0.20, 0.10, 0.0), strict=True):
        direct_label(
            ax, final + 3, y, FACTOR_LABELS[task], FACTOR_COLORS[task], size=9.2, bold=True
        )
    ...
    ax.text(
        43,
        0.90,
        "every factor starts recoverable\n"
    ...
    ax.text(
        43,
        0.62,
        "each path: mean over 3 seeds\nshading: seed min-max",
        fontsize=7.8,
        color=SLATE,
        ha="left",
        va="top",
    ...
    grid_values = np.asarray(
        [
            [mean_range(series[(condition, task, final)])[0] for condition in CONDITION_ORDER]
            for task in tasks
        ]
    )
...
def fig2_geometry_transfer(
    ...
    if associations_csv.is_file():
        intervals = {
            ...
            if row["x"] == "conditional_axis_cosine" and row["y"] == "source_ood_balanced_accuracy"
        }

    models = [name for name in MODEL_ORDER if (eval_root / name).is_dir()]
    fig, axes = plt.subplots(1, len(models), figsize=(10.2, 3.15), sharex=True, sharey=True)
    ...
    n_targets = 0
    for index, (ax, encoder_id) in enumerate(zip(axes, models, strict=True)):
        ...
        ax.scatter(
            ...
            ys,
            s=30,
            marker=MODEL_MARKERS[encoder_id],
            facecolor=color,
            edgecolor="white",
            linewidth=0.5,
            alpha=0.92,
    ...
    fig.supxlabel(
        f"conditional axis alignment   ({n_targets} target attributes; held-out images, "
        f"{shot}-shot probes)",
        fontsize=10.5,
        y=0.035,
    )
...
def fig3_dependence(strata_csv: Path, stem: Path) -> list[Path]:
    ...
    models = [name for name in MODEL_ORDER if any(row["encoder_id"] == name for row in rows)]
    ...
    for index, (ax, (metric, label, sense)) in enumerate(zip(axes, metrics, strict=True)):
...
def _cube_panel(
    ...
    *,
    ...
    for corner, color in enumerate(CORNER_COLORS):
        ax.scatter(
            *observed[corner], s=30, color=color, edgecolor=INK, linewidth=0.6, depthshade=False
        )
        ax.scatter(
            *predicted[corner],
            s=19,
    ...
    ax.text2D(0.5, -0.075, caption, transform=ax.transAxes, ha="center", va="bottom", fontsize=9.6)
...
def fig4_cubes(box_jsons: Sequence[Path], natural_root: Path, stem: Path) -> list[Path]:
    ...
    for slug, title, subtitle in (
        ...
        threshold = _f(
            ...
            .get("fixed_test_criteria", {})
            .get("max_normalized_centroid_rmse"),
            0.25,
        )
        passed = bool(payload.get("headline_criteria_passed", True))
        ...
        natural.append(
            {
                ...
                "caption": f"RMSE {rmse:.3f}    " + rf"max$|\cos|$ {cos:.3f}" + status,
            }
        )
    ...
    handles = [
        Line2D(
            ...
            marker="o",
            ms=6.5,
            markerfacecolor=CORNER_COLORS[1],
            markeredgecolor=INK,
        ...
        Line2D(
            ...
            label="capture-based box prediction",
        ),
    ]
    fig.legend(
        handles=handles,
...
def fig5_model_selection(selection_csvs: Sequence[tuple[str, Path]], stem: Path) -> list[Path]:
    ...
    fig, axes = plt.subplots(1, len(selection_csvs), figsize=(10.2, 3.6), sharey=True)
    axes = list(np.atleast_1d(axes))
    fig.subplots_adjust(left=0.215, right=0.99, top=0.85, bottom=0.17, wspace=0.09)

    for index, (ax, (dataset, path)) in enumerate(zip(axes, selection_csvs, strict=True)):
        ...
        for position, rule in zip(positions, present, strict=True):
            row = rows[rule]
...
#: to 1e4. Raw for all keeps the axis honest. The reference at 1 is the
#: probability ceiling; 0.5 is balanced-class chance error.
BOUND_SERIES = (
    ("empirical", "empirical error", INK, "o", "-"),
    ("thm45_B_raw", "Thm 4.5 plug-in RHS (ours)", AMBER, "s", "-"),
    ("thm41_dir", "Thm 4.1, directional", TEAL, "^", (0, (5, 2))),
    ("luthra2025_optimized_official", "Luthra et al., optimized", MAGENTA, "D", (0, (1, 1.6))),
)


def _bound_reference_lines(ax: Any, *, labels: bool = False, note: str = "") -> None:
    """Mark the probability ceiling and balanced-class chance error."""
    ax.axhline(1.0, color=SLATE, lw=0.9, ls=(0, (4, 3)), zorder=1)
    ax.axhline(0.5, color=MUTE, lw=0.8, ls=(0, (1, 2)), zorder=1)
    if labels:
        ax.text(
            0.98,
            0.95,
            "dashed: probability ceiling (1)\ndotted: balanced chance (0.5)" + note,
            transform=ax.transAxes,
            fontsize=7.4,
            color=SLATE,
            ha="right",
...
def fig7_bounds_shapes3d(
    metrics_json: Path, stem: Path, ranks: Sequence[int] = (8, 32, 64), m_max: int = 2000
) -> list[Path]:
    ...
    fig, axes = plt.subplots(len(rows), len(ranks), figsize=(10.2, 7.8), sharex=True, sharey=True)
    axes = np.atleast_2d(axes)
    fig.subplots_adjust(left=0.085, right=0.995, top=0.93, bottom=0.135, wspace=0.10, hspace=0.30)
    ...
    for row, (task, node) in enumerate(rows):
        for column, rank in enumerate(ranks):
            ...
            ax.set_xscale("log")
            ax.set_yscale("log")
            _bound_reference_lines(
                ax,
                labels=row == 0 and column == 0,
                note="\ncurves shown through m = 2,000",
            )
            heading(
                ax,
                rf"{FACTOR_LABELS.get(task, task)}   $r$={rank}   "
                rf"$B$={cell['B']:.2f}",
                pad=6,
            )
...
def fig6_bounds_from_run(run_root: Path, stem: Path) -> list[Path]:
    ...
    tightness depends strongly on r. Reference lines mark the probability
    ceiling (1.0) and balanced-class chance error (0.5).
    """
    methods = []
    ...
    fig, axes = plt.subplots(
        len(methods), len(ranks), figsize=(10.2, 5.6), sharex=True, sharey=True
    )
    axes = np.atleast_2d(axes)
    fig.subplots_adjust(left=0.085, right=0.995, top=0.90, bottom=0.175, wspace=0.10, hspace=0.28)
    ...
    for row, (label, adaptive) in enumerate(methods):
        for column, rank in enumerate(ranks):
            ...
            ax.set_xscale("log")
            ax.set_yscale("log")
            _bound_reference_lines(ax, labels=row == 0 and column == 0)
...
def figS1_dynamics(survival_csv: Path, stem: Path) -> list[Path]:
    ...
    for row in rows:
        grouped[(row["condition"], row["task"], int(row["epoch"]))].append(_f(row["capture_B"]))

    fig, axes = plt.subplots(1, len(CONDITION_ORDER), figsize=(10.2, 3.0), sharex=True, sharey=True)
    fig.subplots_adjust(left=0.075, right=0.995, top=0.85, bottom=0.20, wspace=0.10)
    for index, (ax, condition) in enumerate(zip(axes, CONDITION_ORDER, strict=True)):
        for task in ("scale", "posX", "posY"):
            means = [mean_range(grouped[(condition, task, epoch)])[0] for epoch in epochs]
            los = [mean_range(grouped[(condition, task, epoch)])[1] for epoch in epochs]
            his = [mean_range(grouped[(condition, task, epoch)])[2] for epoch in epochs]
            color = FACTOR_COLORS[task]
...
def figS2_controls(controls_csv: Path, stem: Path) -> list[Path]:
    ...
    series_specs = (
        ...
        ("supervised_scale_r18_normalized_backbone", "supervised on size only", MAGENTA, "D"),
    )
    ...
    tasks = ("scale", "posX", "posY")
    fig, axes = plt.subplots(1, 2, figsize=(10.2, 3.6), gridspec_kw={"width_ratios": [1.75, 1.0]})
    ...
    for offset, (name, _label, color, _marker) in zip(offsets, series_specs, strict=True):
        shortfall = [max(1.0 - mean_range(capture[(name, task)])[0], 1e-5) for task in tasks]
        ax.bar(positions + offset, shortfall, width * 0.92, color=color, edgecolor="none", zorder=3)
    ax.set_yscale("log")
    ax.set_xticks(positions)
    ...
    for position, (name, _label, color, marker) in zip(order, series_specs, strict=True):
        values = cosine[name]
        if not values:
            continue
        ax.plot([sum(values) / len(values)], [position], marker=marker, ms=9, color=color, zorder=3)
    ax.set_xscale("log")
    ax.set_yticks(order)
    ax.set_yticklabels([])
...
def figS3_natural_summary(summary_csv: Path, stem: Path) -> list[Path]:
    ...
    Capture and axis overlap are 20-resample aggregates. The RMSE point is the
    fixed primary split and its segment is the 20-resample range, not a CI.
    """
    ...
    fig.subplots_adjust(left=0.155, right=0.995, top=0.83, bottom=0.25, wspace=0.30)
    ...
    for position, row in zip(positions, rows, strict=True):
        ...
        ax.plot(
            [low, high], [position, position], color=MUTE, lw=3.0, solid_capstyle="round", zorder=2
        )
    ...
    ax = axes[2]
    for position, row in zip(positions, rows, strict=True):
        ax.plot([_f(row["max_abs_cos"])], [position], marker="o", ms=9, color=BLUE, zorder=3)
    ax.set_yticks(positions)
    ax.set_yticklabels([])
    ax.set_xlim(0, 0.30)
    ...
    fig.text(
        0.995,
        0.02,
        "capture/cosine: 20-resample aggregate; RMSE point: fixed primary split; "
        "segment: 20-resample range",
        fontsize=7.8,
        color=SLATE,
        ha="right",
        va="bottom",
...
def figS4_permutation(permutation_csv: Path, stem: Path) -> list[Path]:
    ...
    p_values = [_f(row["p_value"]) for row in rows]
    if not p_values:
        raise ValueError(f"no permutation rows in {permutation_csv}")
    if not all(math.isclose(value, p_values[0]) for value in p_values):
        p_label = f"p = {min(p_values):.1g}–{max(p_values):.1g}"
    else:
        p_label = f"p = {p_values[0]:.1g}"
    for position, row in zip(positions, rows, strict=True):
        ...
        ax.plot(
            [null_mean], [position], marker="|", ms=14, color=SLATE, markeredgewidth=1.8, zorder=3
        )
        ...
        ax.annotate(
            "",
            xy=(observed + 0.022, position),
            xytext=(null_min - 0.008, position),
    ...
    ax.set_xlim(0, 1.16)
    ax.set_xlabel("normalized corner RMSE")
    heading(ax, f"Every run beats its shuffled null at {p_label}")
    ax.xaxis.grid(True, color=GRID, lw=0.8)
    ax.set_axisbelow(True)
    clean(ax)
...
def figS5_failures(association_csvs: Sequence[tuple[str, Path]], stem: Path) -> list[Path]:
    """Which geometric defects track a drop in transfer."""
    rows: list[dict[str, Any]] = []
    for dataset, path in association_csvs:
        source = _rows(path)
        models = sorted({row["encoder_id"] for row in source})
        for model in models:
            selected = {
                row["x"]: row
                for row in source
                if row["encoder_id"] == model and row["y"] == "source_transfer_gap"
            }
            required = {"interaction_defect_normalized", "midpoint_drift_abs"}
            if not required <= selected.keys():
                continue
            defect = selected["interaction_defect_normalized"]
            drift = selected["midpoint_drift_abs"]
            rows.append(
                {
                    "dataset": dataset,
                    "model": model,
                    "defect_rho": defect["spearman"],
                    "defect_low": defect["ci_low"],
                    "defect_high": defect["ci_high"],
                    "drift_rho": drift["spearman"],
                    "drift_low": drift["ci_low"],
                    "drift_high": drift["ci_high"],
                }
            )
    ...
    for index, (ax, (prefix, label)) in enumerate(zip(axes, specs, strict=True)):
        ...
        if index == 0:
            ...
            ax.set_yticklabels(
                [
                    ...
                    f"{DATASET_LABELS.get(row['dataset'], row['dataset'])} eval"
                    for row in ordered
                ]
            )
        else:
...
def figS6_shots(shot_csvs: Sequence[tuple[str, Path]], stem: Path) -> list[Path]:
    ...
    for index, (ax, (dataset, path)) in enumerate(zip(axes, shot_csvs, strict=True)):
        rows = _rows(path)
        ...
        for encoder_id in MODEL_ORDER:
            ...
            los = [
                values[i] - _f(entries[shot]["source_ood_balanced_accuracy_ci_low"])
                for i, shot in enumerate(shots)
            ]
            his = [
                _f(entries[shot]["source_ood_balanced_accuracy_ci_high"]) - values[i]
                for i, shot in enumerate(shots)
            ]
            color = MODEL_COLORS[encoder_id]
            ax.errorbar(
