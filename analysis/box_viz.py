...
def plot_box_3d(coords, box, granular_task, triple_names, save_path,
                ...
                sample_size=9, sample_alpha=0.38):
    ...
    if show_samples:
        for idx in range(8):
            ...
            ax.scatter(p[sel, 0], p[sel, 1], p[sel, 2],
                       s=sample_size, alpha=sample_alpha,
    ...
    if show_centroid_se:
        for entry in box:
            ctr = entry.get("center")
            se = entry.get("center_se")
            if ctr is None or se is None:
                continue
            ax.errorbar(
                ctr[0], ctr[1], ctr[2],
                xerr=se[0], yerr=se[1], zerr=se[2],
                fmt="none", ecolor="0.25", elinewidth=1.0, capsize=2.0,
                alpha=0.8,
            )
