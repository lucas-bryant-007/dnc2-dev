...
Obs(f) is estimated as the held-out R^2 of a flexible finite MLP regressor
...
{
  "factors": [
    {"name": "object x-position", "obs": 0.98, "obs_sd": 0.01,
                                  "cap": 0.95, "cap_sd": 0.02},
    ...
  ],
  "sweep": {
...
def panel_a(ax, factors):
    ...
    obs_sd = np.array([f.get("obs_sd", f.get("obs_se", 0.0)) for f in factors])
    cap_sd = np.array([f.get("cap_sd", f.get("cap_se", 0.0)) for f in factors])
    ...
    ax.bar(x, obs, w, facecolor="none", edgecolor=DARK, linewidth=1.1,
           linestyle="--", label=r"$\widehat{\mathrm{Obs}}(f)$  (finite MLP estimate)",
           zorder=2)
    ax.errorbar(x, obs, yerr=obs_sd, fmt="none", ecolor=DARK, capsize=2,
    ...
    ax.errorbar(x, cap, yerr=cap_sd, fmt="none", ecolor=DARK, capsize=2,
                elinewidth=0.8, zorder=5)

    for xi, o, c in zip(x, obs, cap, strict=True):
        r = regime(o, c)
...
def panel_b(ax, sweep):
    ...
    obs_sd = np.array(sweep.get("obs_sd", sweep.get("obs_se", np.zeros_like(obs))))
    cap_sd = np.array(sweep.get("cap_sd", sweep.get("cap_se", np.zeros_like(cap))))

    ax.fill_between(H, cap, obs, color=AMBER, alpha=0.13, lw=0,
                    label="compression gap")
    ax.plot(H, obs, "--o", color=DARK, ms=3, lw=1.1, label=r"$\widehat{\mathrm{Obs}}(f)$")
    ax.fill_between(H, obs - obs_sd, obs + obs_sd, color=DARK, alpha=0.15, lw=0)
    ax.plot(H, cap, "-o", color=AMBER, ms=3, lw=1.3, label=r"$B_f(G)$")
    ax.fill_between(H, cap - cap_sd, cap + cap_sd, color=AMBER, alpha=0.20, lw=0)
...
DEMO = {
    "_synthetic": True,
    "factors": [
        {"name": "object x-position", "obs": 0.97, "obs_sd": 0.01, "cap": 0.93, "cap_sd": 0.02},
        {"name": "object y-position", "obs": 0.96, "obs_sd": 0.01, "cap": 0.91, "cap_sd": 0.02},
        {"name": "T orientation", "obs": 0.71, "obs_sd": 0.04, "cap": 0.14, "cap_sd": 0.03},
        {"name": "goal progress", "obs": 0.09, "obs_sd": 0.03, "cap": 0.01, "cap_sd": 0.01},
    ],
    "sweep": {
        "factor": "T orientation",
        "H": [1, 2, 4, 8, 16],
        "obs": [0.94, 0.88, 0.71, 0.42, 0.16],
        "obs_sd": [0.01, 0.02, 0.04, 0.05, 0.04],
        "cap": [0.61, 0.44, 0.14, 0.06, 0.02],
        "cap_sd": [0.04, 0.04, 0.03, 0.02, 0.01],
    },
    "n_seeds": 5,
    "bottleneck_r": 16,
}
