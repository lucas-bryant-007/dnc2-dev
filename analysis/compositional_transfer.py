...
def _load_encoder(args: argparse.Namespace):
    ...
    if args.encoder_kind == "vicreg_imagenet1k":
        if not args.weights_path:
            raise ValueError(
                "--weights-path is required for encoder-kind=vicreg_imagenet1k"
            )
        from models.public_weights import load_vicreg_imagenet_resnet50

        model = load_vicreg_imagenet_resnet50(
            args.weights_path, device=args.device
        )
        return model, {
            "kind": args.encoder_kind,
    ...
    if args.encoder_kind == "supervised_imagenet1k":
        if not args.weights_path:
            raise ValueError(
                "--weights-path is required for encoder-kind=supervised_imagenet1k"
            )
        ...
        model = load_supervised_imagenet_resnet50(
            args.weights_path, device=args.device
        )
        return model, {
            "kind": args.encoder_kind,
            ...
            "weights_url": PUBLIC_WEIGHTS["supervised-imagenet"].url,
            "weights": args.weights_path,
            "weights_sha256": _sha256_if_file(args.weights_path),
        }
