...
- The clarified Figure 1 renderer is packaged in the new immutable
  `paper_release_20260825c`, whose complete manifest validates. The superseded
...
| Audit-time test suite | 149 passed before handoff pruning | The collaborator branch intentionally omits test files; current acceptance checks are Ruff, shell syntax, byte-level artifact manifests, and model-asset load validation. |
...
| Figure release | `analysis/build_paper_release.py`; `analysis/paper_figures_v2.py` | Seven main and six supplement figures | Artifact-verified in `paper_release_20260825c` |
...
1. **Figure 1 clarification.** The new renderer and
   ...
   is included in `paper_release_20260825c`.
...
3. **Manuscript repair.** `docs/manuscript_repairs.md` contains source-ready
   ...
   source.
4. **Fresh immutable release.** `paper_release_20260825c` is the current review
...
`paper_outputs/paper_release_20260825c/`. Superseded full snapshots are omitted
...
The current `paper_release_20260825c` passes the following checks:
...
- `paper_outputs/hyperrectangle_review_20260825b/MANIFEST.csv`: 21/21 valid;
