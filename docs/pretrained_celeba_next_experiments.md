...
The triple is selected once from training data. Test labels are then used only to
construct the declared uniform eight-cell evaluation population. Never use
`--allow_constraint_fallback` for a headline run.

## Shared server setup

```bash
cd ~/dnc2_s2/dnc2_work/dnc2-dev
git pull --ff-only origin main

export PY=/home/lucas_bryant1/dnc2_s2/dnc2_env/bin/python
export HF_HOME="$HOME/dnc2_s2/cache/huggingface"
export HF_DATASETS_CACHE="$HOME/dnc2_s2/cache/huggingface/datasets"
export TORCH_HOME="$HOME/dnc2_s2/cache/torch"
...
```bash
export GPU=0
export CKPT="$HOME/dnc2_s2/hf_models/vicreg-resnet50-celeba/converted_checkpoints"
export OUT="$HOME/dnc2_s2/results/pretrained_20260722/celeba_vicreg_stability"
...
```bash
JSON=$(find "$OUT/metrics" -maxdepth 1 -type f -name '*stability_20x500.json' -print -quit)
...
spread and ample margin from every preregistered threshold.
...
```bash
export ORIGINAL="$HOME/dnc2_s2/hf_models/ijepa-resnet50-celeba/converted_checkpoints/epoch_1000.ckpt"
export REPAIRED="$HOME/dnc2_s2/hf_models/ijepa-resnet50-celeba/repaired_checkpoints/epoch_1000.ckpt"
export IOUT="$HOME/dnc2_s2/results/pretrained_20260722/celeba_ijepa_stability"
mkdir -p "$IOUT/logs"
